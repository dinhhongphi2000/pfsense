...
/*if account signed then return error*/
if (($_POST['auth_user'] && $_POST['auth_pass']) || $_POST['auth_voucher'] ){
	if($_POST['auth_user'])
		$cpsession = captiveportal_isusername_logged($_POST['auth_user']);
	else 
		$cpsession = captiveportal_isusername_logged($_POST['auth_voucher']);
       /* echo $_POST['auth_user'];
        ob_flush();
        return;*/
	if(!empty($cpsession)){
		$error_message = "Tài khoản đã được sử dụng";
...
if ((!empty($cpsession)) && (! $_POST['logout_id']) && (!empty($cpcfg['page']['logouttext']))) {
	...
	show_logout_page($sessionid,$cpzone);
	ob_flush();
	return;
...
function show_logout_page($seesionid,$cpzone){
	...
	$htmltext = get_include_contents("{$g['varetc_path']}/captiveportal-{$cpzone}-logout.html");

	$htmltext = str_replace("\$PORTAL_ACTION\$", "{$protocol}{$ourhostname}/index.php?logout_id={$seesionid}", $htmltext);
	
	$htmltext = str_replace("\$CLIENTID\$", htmlspecialchars($seesionid), $htmltext);
	$htmltext = str_replace("\$PORTAL_ZONE\$", htmlspecialchars($cpzone), $htmltext);

	echo $htmltext;
}

function captiveportal_isusername_logged($username) {
	global $g, $cpzone;
